...
# reds = [(((x-x_min)//scale)+1,((y-y_min)//scale)+1) for x,y in reds]
# reds = [(round((x-x_min)/scale)+1,((y-y_min)//scale)+1) for x,y in reds]
# reds = [(((x-x_min)//scale)+1,round((y-y_min)/scale)+1) for x,y in reds]
...
print(f'{found} cnt = {cnt}, found area = {area}')
...
# %% unsure
    ...
    Ys_in_range_mins = []
    Ys_in_range_maxs = []
    for x in range(x_rect[0],x_rect[1]+1):
        ind_one_x = [index for index, value in enumerate(Xs) if value == x_rect[0]]
        Ys_at_one_x = [Ys[i] for i in ind_one_x]
        Ys_in_range_mins.append(min(Ys_at_one_x))
        Ys_in_range_maxs.append(max(Ys_at_one_x))
    Ys_critical = (max(Ys_in_range_mins), min(Ys_in_range_maxs))
    if Ys_critical[0] <= x_rect[0] and Ys_critical[1] >= x_rect[1]:
        found = True
    else:
        found = False

    if found: # only check Y-dim if it's worth it
# check that the reds are at least at the boundary of the rectangle
    Xs_in_range_mins = []
    Xs_in_range_maxs = []
    for i in range(indy):
        ind_one_y = [index for index, value in enumerate(Ys) if value == y_rect[0]]
        Ys_at_one_x = [Ys[i] for i in ind_one_x]
        Ys_in_range_mins.append(min(Ys_at_one_x))
        Ys_in_range_maxs.append(max(Ys_at_one_x))
    Ys_critical = (max(Ys_in_range_mins), min(Ys_in_range_maxs))
...
print(f'cnt = {cnt}, found area = {area}')
...
# True cnt = 90000+663, found area = 1702088750 (not right) 12/23/2025
